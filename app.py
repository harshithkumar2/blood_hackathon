...
#         if admin:
#             pas_check = sha256_crypt.verify(self.password,admin.password)
#             if pas_check:
...
def Admindash():
    if 'admin' in session:
        ...
        return render_template('admin_dash.html',data=[hospital,donar,bank])
    return redirect(url_for("Adminlog"))
...
def bloodbank_dash():
    if 'blood_bank' in session:
        ...
        donar = Donor.query.count()
        bank = Blood_bank.query.count()
        return render_template("bloodbank_dash.html",data=[recep,donar,bank])
            ...
            if name_check:
                ...
                if not name_check and not phno_check and not email_check:
                    ...
                    if address_check:
                        ...
                        db.session.commit()
                        flash("Donor details updated successfully","success")
                        return redirect(url_for("donor_list"))9
    else:
        flash("Please login as a admin","error")
        return redirect(url_for("Adminlog"))
